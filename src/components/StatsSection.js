const StatsSection = ({ totalVolume, totalUsers }) => {
  return (
    <div className="bg-black opacity-90 text-white py-8 mb-4">
      <div className="container mx-auto grid grid-cols-2 gap-4 md:grid-cols-2 text-center">
        ...
        <div className="flex flex-col items-center space-y-2">
          <div className="flex items-center text-2xl md:text-4xl font-bold">
            {totalVolume?.toLocaleString()} <span className="ml-2">CORE</span>
          </div>
          <span className="text-gray-400 text-sm uppercase">Total Volume</span>
        </div>
