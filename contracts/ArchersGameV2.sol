...
contract ArchersGame is Ownable, ReentrancyGuard {
    ...
    event PowerUpPlaced(address indexed player, uint256 amount, string side);
    ...
    function _placePowerUp(string memory side, uint256 amount) internal {
        require(amount >= minimumAmount, "Power Up amount must be greater than or equal minimum amount");
        ...
        emit PowerUpPlaced(msg.sender, msg.value, side);
    }
